 ...
 * 
 * The AIPlayer class represents an AI-controlled player in the Cabo game. This class extends the
 * Player class and adds additional functionality for AI logic, such as tracking card knowledge and
 * calculating a "blind" hand value.
 * 
 * @author Yiheng Su and Yikai Zhang
 */
public class AIPlayer extends Player {

  private final boolean[][] cardKnowledge; // Tracks whether the player knows the rank of a card in
                                           // the game

  /**
   * Constructs an AIPlayer with the specified name, label, and computer status. The AI player
   * tracks knowledge of its own cards, marking the first two cards in its hand as known.
   *
  ...
  /**
   * Calculates the "blind" total value of the AI player's hand. If the AI doesn't know the value of
   * a card in its hand, it assumes an average value of 8 for that card. Otherwise, it uses the
   * actual rank of the known cards.
   *
  ...
  /**
   * Returns the index of the first card in the AI's hand that is unknown. The method checks the
   * card knowledge array and identifies the first card whose rank is unknown to the AI.
   *
  ...
  /**
   * Returns the index of the first card in the human player's hand that is unknown to the AI. The
   * AI can use this index for spying on an unknown card in the human player's hand.
   *
   * @return the index of the first unknown card in the human player's hand, or -1 if all cards are
   *         known.
   */
  ...
  /**
   * Returns the index of the lowest-ranked card in the specified player's hand that is known to the
   * AI. The method searches for the card with the lowest rank in the player's hand where the AI has
   * card knowledge.
   *
  ...
  /**
   * Sets the knowledge of a specific card for the AI or player. This method updates whether the AI
   * or player knows the value of a card at the given index in a player's hand.
   *
   ...
   * @param knowledge {@code true} if the AI or player knows the card's value, {@code false}
   *                  otherwise.
   */
  ...
  /**
   * Returns the index of the highest-ranked card in the AI's hand that is known. The method
   * searches for the card with the highest rank in the AI's hand, considering only the cards that
   * the AI knows. If no card has a rank greater than or equal to 8, the method returns -1.
   *
   * @return the index of the highest-ranked known card in the AI's hand, or -1 if no suitable card
   *         is found.
   */
